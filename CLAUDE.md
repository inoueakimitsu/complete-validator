...
- `tests/test_harness.py` は、static fixture annotation から集計した `rule_metrics` (rule ごとの F1 / disruption / support) を用いて `rule_recommendations` を生成する。
- 生成時は guardrail (`max_f1_drop`, `max_disruption_increase`) と `min_support` で rule を選別し、条件を満たす rule のみ候補 config 差分を提案する。
...
- 現状:
  ...
  - `rule_recommendations` は static annotation 由来の `rule_metrics` を使って自動生成される。rule ごとの F1/disruption/support を見て guardrail を満たす rule のみに限定して提案する。
  - `rule_metrics` が存在しない run では、後方互換のためブロードキャストにフォールバックする。
- 方針:
...
- 実装済み:
  ...
  - `rule_metrics` ベースの `rule_recommendations` 自動生成 (F1/disruption/support の guardrail 付き)
  - `rule_metrics` 未提供時のブロードキャスト フォールバック
...
- 方針 (未実装):
  - shadow 推奨結果 (`rule_recommendations`) の意味解析ベース高精度化 (現状は rule-level confusion/F1/disruption による統計選別)
