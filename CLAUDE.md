...
python3 tests/test_harness.py --scenario dynamic --config tests/configs/baseline.json --lock-unlock-hysteresis 2
...
Claude Code の TUI を tmux の `send-keys` と `capture-pane` で操作し、**ローカル plugin を使った E2E** を実行できます。  
ここでは、実際に詰まりやすかったポイントを前提に、初見でも再現しやすい手順を記載します。

#### 先に結論

- GitHub へ push しなくても E2E は可能です。
- **最優先の再現手順は `--plugin-dir`** です (marketplace/install UI 操作をスキップできる)。
- ローカル更新反映の必須条件は次の 3 つです。
  - plugin version を更新
  - `/plugin` で marketplace update/install
  - Claude 再起動
- deny 検証は「事前変更済み」+「`git add && git commit` のみ」で実行します。
- tmux 自動化では、最初に表示される `Yes, I trust this folder` を **Enter で通過**しないと、その後のコマンド送信は無効になります。

#### E2E 依頼を受けたときの標準オペレーション (Runbook)

`「E2E テストして」` と依頼されたら、次の順で進めます。

1. 開始前チェック
 - `claude --version` を記録する。
 - `claude --help` で `--plugin-dir` が使えることを確認する。

2. 方式選択
 - 原則は `--plugin-dir` 方式を使う (再現性優先)。
 - marketplace/UI 手順は、`--plugin-dir` で切り分け不能なときだけ使う。

3. 旧状態の掃除 (必要時)
 - `already installed` が続く、または hook が発火しない場合のみ実施。
 - `uninstall(local/project) -> marketplace remove -> add/update/install -> Claude再起動` を行う。

4. テスト repo 準備
 - 最小 deny ルールを作る。
 - 初回 commit を作成。
 - deny 対象の変更 (`E2E_BLOCK` など) は **先にファイルへ反映**する。

5. Claude TUI 起動 (tmux)
 - `env -u CLAUDECODE claude --plugin-dir ... --permission-mode dontAsk --allowedTools 'Bash(git:*)' --debug-file ...`
 - `--allowedTools` は必ずクォートする。

6. 初期画面ハンドリング
 - `Yes, I trust this folder` が出たら `Enter`。
 - `❯` プロンプトが出るまで待機する。

7. 実行
 - Claude には `git add <target> && git commit -m <msg>` のみ送る。
 - 入力欄に出るだけで動かない場合は `Enter` を再送し、`● Bash(...)` 出現で実行開始を確認する。

8. 成功判定
 - TUI: `Blocked by hook` / `Hook PreToolUse:Bash denied this tool`
 - debug: `Getting matching hook commands for PreToolUse with query: Bash` と `permissionDecision: "deny"`
 - どちらか片方だけではなく、原則両方を確認する。

9. 失敗時分岐
 - `No items match`: `/plugin` 検索欄事故。`Esc` で戻って再送。
 - `syntax error near unexpected token '('`: `--allowedTools` 未クォート。
 - `Found 0 hook matchers`: plugin未読込。方式/再起動/インストール状態を再確認。
 - commitが通る: 変更作成タイミングが誤り (`echo && add && commit` 一体化) の可能性を最初に疑う。

10. 終了処理
 - 一時 repo / tmux session を削除。
 - 結果報告は「実行コマンド」「判定ログ」「失敗時の再現原因」をセットで残す。

#### ローカル反映のチェックポイント

- debug ログで以下を確認します。
  - `Using manifest version for complete-validator@complete-validator: <version>`
  - `Loaded hooks from .../plugins/cache/complete-validator/complete-validator/<version>/hooks/hooks.json`
- hook 動作は以下で確認します。
  - `executePreToolHooks called for tool: Bash`
  - `Found N hook matchers in settings` (`N >= 1`)
  - `Matched N unique hooks for query "Bash"`

#### 失敗しない tmux 操作ルール

- 文字列送信と `Enter` を同時に送らない。
- 各操作の前に `capture-pane` で画面状態を確認する。
- 応答中 (`...ing`) は送信しない。
- slash コマンドは 1 つずつ送る。
- 画面が想定と違うときは `Esc` で戻る。
- `/plugin` 画面の検索欄にコマンド文字列を流し込まない。
- `send-keys ... C-m` 直後でも入力が未送信のまま残ることがある。`❯` 行に文面だけが残って反応しない場合は `Enter` を追加で 1 回送る。

#### 最小 E2E 手順 (push 不要, tmux + `--plugin-dir`)

```bash
# 1) テスト repo
PLUGIN_DIR=/mnt/c/Users/inoue.akimitsu/git/complete-validator
tmp=/tmp/test-cv
rm -rf "$tmp"
mkdir -p "$tmp/.complete-validator/rules"
git -C "$tmp" init -q
git -C "$tmp" config user.name "cv-e2e"
git -C "$tmp" config user.email "cv-e2e@example.com"
cat > "$tmp/.complete-validator/rules/e2e_block.md" <<'EOF'
---
applies_to:
  - "*.txt"
---
...
EOF
echo "initial" > "$tmp/note.txt"
git -C "$tmp" add .
git -C "$tmp" commit -qm "init"

# 2) Claude 起動 (plugin-dir 指定)
tmux new-session -d -s test-cv -c "$tmp" -x 200 -y 50
tmux send-keys -t test-cv "env -u CLAUDECODE claude --plugin-dir $PLUGIN_DIR --permission-mode dontAsk --allowedTools 'Bash(git:*)' --debug-file $tmp/claude_tui_debug.log"
sleep 0.5
tmux send-keys -t test-cv Enter
sleep 8

# 3) trust prompt が出たら Enter で通過
tmux capture-pane -t test-cv -p -S -120 | rg -n "Yes, I trust this folder" && tmux send-keys -t test-cv Enter

# 4) 先に変更を作る (重要)
echo "E2E_BLOCK" >> "$tmp/note.txt"

# 5) Claude には add/commit だけ実行させる
tmux send-keys -t test-cv "git add note.txt && git commit -m e2e-check"
sleep 0.5
tmux send-keys -t test-cv Enter
sleep 15
tmux capture-pane -t test-cv -p -S -160
tmux capture-pane -t test-cv -p -S -220 | rg -n "Blocked by hook|denied this tool|PreToolUse:Bash hook returned blocking error"
rg -n "Getting matching hook commands for PreToolUse|Found 1 hook matchers|permissionDecision|Hook PreToolUse:Bash" "$tmp/claude_tui_debug.log"
```
...
- `Loaded hooks from standard location for plugin complete-validator: .../hooks/hooks.json`
- `Getting matching hook commands for PreToolUse with query: Bash`
- `Found N hook matchers in settings` (`N >= 1`)
- `\"permissionDecision\": \"deny\"`
- `Hook PreToolUse:Bash (PreToolUse) success`

#### `--plugin-dir` を使う E2E (推奨, marketplace 操作不要)

plugin の install/update UI 操作に依存せず、**この session のみ**ローカル plugin を読み込んで E2E できます。
...
(
  ...
  env -u CLAUDECODE claude \
    ...
    --debug-file "$dbg" \
    -p "Execute exactly: git add note.txt && git commit -m e2e-plugin-dir-dontask."
)
```

成功判定:

- `claude_debug.log` に以下が出ること
  - `Getting matching hook commands for PreToolUse with query: Bash`
  - `Hook PreToolUse:Bash (PreToolUse) success`
  - `\"permissionDecision\": \"deny\"`
...
- `--plugin-dir` 方式は session 限定ロードのため、既存 marketplace/install 状態の影響を受けにくい。
- `--debug-file` を必ず併用すると、「hook 未発火」か「hook 発火後 allow/deny」かの切り分けができる。
- `Found 0 hook matchers in settings` が startup (`SessionStart`) で出ても、それだけでは失敗ではない。`PreToolUse` 実行時ログで判定する。
- deny 確認時の実ログは `Getting matching hook commands for PreToolUse with query: Bash` -> `Found N hook matchers in settings` -> `permissionDecision: \"deny\"` -> `Hook PreToolUse:Bash (PreToolUse) success` の順で確認する。

#### `send-keys` の実践パターン

```bash
# Good
tmux send-keys -t test-cv "/plugin"
sleep 0.5
tmux send-keys -t test-cv Enter

# Bad
tmux send-keys -t test-cv "/plugin" Enter
```

```bash
# `--allowedTools Bash(git:*)` は必ずクォートする
# (未クォートだと shell が `(` `)` を解釈して Claude が起動しない)
tmux send-keys -t test-cv "env -u CLAUDECODE claude --allowedTools 'Bash(git:*)'"
sleep 0.5
tmux send-keys -t test-cv Enter
```
...
sleep 0.5
tmux capture-pane -t test-cv -p -S -40
```

```bash
# 入力欄にコマンドが見えても実行が始まらない場合がある
# `● Bash(...)` が出ないときは Enter を再送する
tmux send-keys -t test-cv "git add note.txt && git commit -m e2e-check"
sleep 0.5
tmux send-keys -t test-cv Enter
sleep 1
tmux capture-pane -t test-cv -p -S -80 | rg -n "● Bash\\("
```

#### `/plugin` 画面の update/install 手順

1. `Marketplaces` タブで `complete-validator` を選択
2. `u` で marketplace update
3. `Enter` で marketplace details
4. `Browse plugins` を開く
5. `Install` を実行
6. `Installed` タブで対象 plugin に `i` を実行
7. 必要なら `project scope` を `Down` + `Enter` で選択

#### plugin を確実に入れ直すハードリセット手順

`already installed` が続く、または hook が発火しない場合は、次の順に完全に入れ直します。

```bash
/plugin uninstall complete-validator@complete-validator --scope local
/plugin uninstall complete-validator@complete-validator --scope project
/plugin marketplace remove complete-validator
/plugin marketplace add /mnt/c/Users/inoue.akimitsu/git/complete-validator
/plugin marketplace update complete-validator
/plugin install complete-validator@complete-validator --scope local
```

その後は **Claude Code を再起動** してから E2E を再実行します。

運用メモ:

- 途中で `/plugin` 画面の検索欄に入力が吸われると `No items match \"...\"` になります。
- この状態では slash command が実行されていないため、`Esc` でチャット入力に戻してから再送してください。
- 自動化時は、各 command の前に `Esc` を送ると事故を減らせます。

#### よくある詰まりと対処

- 症状: `lugin install ...` のように先頭 `/` が欠ける
  - 対処: 応答完了後に送る。`❯` が待機状態か確認。
- 症状: `Accessing workspace` で止まる / `Yes, I trust this folder` が表示されたまま
  - 対処: `Enter` で trust prompt を確定する。これを通過しないと後続コマンドは実行されない。
- 症状: `No items match "..."`
  - 対処: `/plugin` の検索欄に誤入力されている。`Esc` で戻る。
- 症状: install したのに旧 version のまま
  - 対処: version 更新 -> marketplace update -> install -> Claude 再起動 -> debug 確認。
- 症状: `-bash: syntax error near unexpected token '('`
  - 対処: 起動コマンドの `--allowedTools` 引数を `'Bash(git:*)'` のようにクォートする。
- 症状: `git add && git commit` が入力欄に出るだけで実行されない
  - 対処: `Enter` を再送し、`● Bash(...)` が出るまで待つ。
- 症状: `Found 0 hook matchers in settings`
  - 対処: plugin が読み込まれていない。install と再起動をやり直す。
- 症状: `echo ... && git add && git commit` で deny にならない
  - 対処: PreToolUse は実行前評価。変更作成を別コマンドで先に行い、commit コマンドは `git add && git commit` のみにする。
...
- **hook の deny 動作を厳密に検証する E2E では `--dangerously-skip-permissions` を使わないことを推奨**します。
- 実測では、このフラグ有効時に commit がそのまま通過し、`Blocked by hook` を再現できないケースがありました。
- 「hook が正しく deny するか」を確認する回は、通常起動 (`claude`) で、`--permission-mode dontAsk` + `--allowedTools 'Bash(git:*)'` を使って実行してください。
...
| Claude Code 起動 | 8 秒 |
| `/plugin` コマンド | 5-10 秒 |
| `git add && git commit` (hook なし) | 10-30 秒 |
| `git add && git commit` (hook あり、キャッシュ ミス) | 1-5 分 |
| `git add && git commit` (hook あり、キャッシュ ヒット) | 20-60 秒 |

#### 結果収集

```bash
tmux capture-pane -t test-cv -p -S -80
tmux capture-pane -t test-cv -p -S -120 | rg -n "Blocked by hook|denied this tool"
```

#### Windows では WSL 内で実行してください

`check_style.sh` は `python3` を使用します。Windows の MSYS2/Git Bash 環境では `python3` が利用できないことがあるため、Windows で tmux E2E テストを行う場合は WSL 内で実行してください。Linux/macOS ではそのまま実行できます。

```bash
wsl bash -c 'tmux new-session -d -s test-cv -c /tmp/test-cv -x 200 -y 50'
wsl bash -c 'tmux send-keys -t test-cv "env -u CLAUDECODE claude --dangerously-skip-permissions"; sleep 0.5; tmux send-keys -t test-cv Enter'
wsl bash -c 'sleep 10 && tmux capture-pane -t test-cv -p -S -50'
```

#### バージョン更新の基本手順 (ローカル marketplace)

```bash
# plugin 側
# 1) .claude-plugin/plugin.json の version を更新
# 2) .claude-plugin/marketplace.json の version を更新

# Claude 側 (/plugin or slash commands)
/plugin marketplace update complete-validator
/plugin install complete-validator@complete-validator --scope project
# -> "Restart Claude Code to load new plugins."
```

再起動後、`~/.claude/plugins/cache/complete-validator/complete-validator/` に期待 version が生成されていることを確認します。

#### deny 後のループは長いので目的を分ける

- 目的が「hook 発火確認」のみなら、最初の deny で終了してよいです。
- 目的が「最終 commit 成功」なら、修正 -> 再 commit ループで時間がかかります。
...
- `cross_file: true` の Python ルールは `dependency_scope` で再チェック範囲を選択できる。
  - `python_imports`: 変更ファイルに依存する Python ファイルを推移閉包で拡張する。
  - `python_imports_direct`: 変更ファイルの直接依存元のみを再チェック対象に追加する。
  - 未指定 (`""`) は後方互換のため `python_imports` と同義。
- `batching` / `context_level` / `cache` は、`scripts/check_style.py` の実行経路で参照される。
- `context_level`:
  - `diff`: 変更差分中心
  - `full_file`: ファイル全体を主対象
  - `smart`: diff 量に応じて `diff/full_file` を切替
- `cache`: `false` の場合、キャッシュ read/write を無効化
- `batching`: `true` の場合、ルール×ファイル単位をファイル基準で並べ替えて実行 (最小バッチ最適化)
- `tests/test_harness.py` は 2 config 比較時に shadow 比較結果 (`shadow_<scenario>__<current>_vs_<candidate>.json`) を保存する。
- 同時に、ガードレール (`F1 drop` / `disruption increase`) とコスト改善 (`wall_time` / `llm_calls`) を用いた推奨判定を `shadow_recommendation_<scenario>__<current>_vs_<candidate>.json` に保存する。
- 同ファイルには `evidence_comparison` (root raw metrics の比較配列) を保存し、根拠単位の生データ (`current/candidate/delta/threshold_ref/samples`) を追跡できる。
- `samples` には集約元の `rule × file` サンプルを保持する (`fixture`, `file`, `expected`, `predicted`, `message`)。
- `message` は `rule_results.message` をそのまま保存し、パーサで切り落とした要約文ではなくブロック全文を保持する。
- `accepted/rejected/insufficient_support` のような解釈済みラベルは `shadow_recommendation` 出力に含めない。採否判断はユーザーまたは上位 AI が raw 比較値から行う。
- `--approve-shadow-recommendation` 指定時のみ、推奨が adopt の場合に `rule-config.json` へ選択プロファイルを反映し、`decision_log` へ監査エントリを追記する。
- 推奨 payload に `rule_recommendations` (ルールキー -> 設定差分 dict) が含まれる場合、承認時に `rule-config.rules` へルール単位でマージ反映する。
- `tests/test_harness.py` は、static fixture annotation から `rule_metrics` と `evidence_metrics` を集計し、`rule_recommendations` を生成する。
- `evidence_metrics` は `rule#evidence_category` 粒度の F1/disruption/support を表し、候補設定の採否を根拠単位で判定する。
- 生成時は guardrail (`max_f1_drop`, `max_disruption_increase`) と `min_support` / `min_evidence_support` で選別し、条件を満たす rule のみ候補 config 差分を提案する。
- `rule_metrics` が不足する場合は後方互換としてブロードキャストにフォールバックする。
- したがって、この3キーは `check_style.py` の実行経路を切り替えるが、値変更だけで最適化効果を主張してはいけない。効果判定はハーネス比較 (`wall_time` / `llm_calls` / `F1`) で行う。
...
- 現状:
  ...
  - 2 config 比較時に shadow 推奨判定を出力する (`adopt_candidate`, `guardrail_passed`, `cost_improved`, `reasons`)。
  - 承認フラグあり (`--approve-shadow-recommendation`) の run では、推奨が採用条件を満たす場合に `.complete-validator/rule-config.json` へ `__harness__/selected_profile` を保存する。
  - さらに `rule_recommendations` があれば、`rules[<rule_key>]` へ差分をマージして反映する。
  - `rule_recommendations` は static annotation 由来の `rule_metrics` + `evidence_metrics` で自動生成される。evidence 粒度で guardrail を判定し、採用可能な rule のみに提案する。
  - `evidence_metrics` が不足する場合は rule 粒度判定にフォールバックし、`rule_metrics` も不足する run では後方互換のためブロードキャストにフォールバックする。
  - `shadow_recommendation` 出力は解釈ラベル (`accepted/rejected/insufficient_support`) ではなく raw 比較 (`evidence_comparison`) を正として扱う。
  - `evidence_comparison.samples` に集約元 `rule × file` サンプルを保持し、後から根拠をトレースできるようにする。
- 方針:
...
- 現状:
  - 実行単位は 1 ルール × 1 ファイル。
  - `cross_file: true` の Python ルールは、変更ファイルから Python import 逆引き依存をたどって再チェック範囲を拡張する。
  - `dependency_scope`:
    - `python_imports`: 推移閉包 (`A <- B <- C` なら `B`, `C` まで含む)
    - `python_imports_direct`: 1-hop のみ (`A <- B` の `B` のみ)
  - `python_imports_direct` は過剰再チェックを抑えるための低コストモードで、広域波及の見逃しリスクとトレードオフになる。
- 未実装:
  - `python_imports` / `python_imports_direct` 以外の dependency scope (例: TS project reference 連鎖)。
  - 言語横断の依存解決と重み付き優先度制御。

### 9. lock_on_satisfy のアンロックとヒステリシス

- 実装済み:
  - dynamic 評価では `lock_on_satisfy` により satisfied 状態を step 間保持する。
  - ロック済みルールが deny になった場合、`--lock-unlock-hysteresis` で指定した連続 deny 回数に達するとアンロックする。
  - 閾値未満の deny はロック維持として `allow` 扱いにすることで、短周期振動を抑える。
  - annotation の `unlock_on_change_keywords` に一致する変更が step に含まれる場合は、ヒステリシス待ちをせず即時アンロックする。
  - annotation の `unlock_on_change_symbols` に一致する識別子変更が step に含まれる場合も、即時アンロックする。
  - Python 追記テキストでは AST 抽出を併用し、関数名/クラス名/属性名ベースの識別子変更も即時アンロック判定に使う。
  - lock 時に `lock_evidence_terms` を rule ごとに保持する。これは「なぜ lock されたか」の最小表現で、rule 名・対象 path・メッセージから抽出した正規化語を保存する。
  - 変更適用時は `append_text` から抽出した語を正規化し、`lock_evidence_terms` との交差を判定する。交差ありなら即時アンロックする。
  - 語の正規化では軽量な同義語マップを使う (`authentication -> auth`, `credentials -> token` など)。これにより表層語が一致しなくても根拠変化を拾える。
  - アンロック時は `deny_streak` と同時に該当 rule の `lock_evidence_terms` も破棄し、古い根拠が次 step に残らないようにする。
- 未実装:
  - ルール本文と対応づけた AST/span レベルの根拠追跡 (どの predicate が満たされたか)。
  - import 依存を含む cross-file 根拠伝播と、影響集合ベースの厳密 invalidation。
  - semantic embedding / entailment を使った高精度な意味一致判定。

#### 9.1 lock_evidence_terms の設計意図

- 目的:
  - `lock_on_satisfy` の即時アンロックを「単発キーワード一致」から一段進め、最低限の根拠状態を持たせる。
- 背景:
  - 以前は `unlock_on_change_keywords` / `unlock_on_change_symbols` を明示していない rule では、根拠変更の検知が deny 連続カウント頼みだった。
  - その結果、意味的には根拠が変わっていても lock が残りやすく、fixpoint の反復回数が増えるケースがあった。
- 選んだトレードオフ:
  - 完全な意味解析 (AST span + predicate) は重いため、Phase 1 は「正規化語集合の交差」で近似する。
  - これにより実装複雑性を抑えつつ、明示 keyword がない rule でも即時アンロックの入口を作れる。
- 失敗モード:
  - 語彙抽出に依存するため、暗黙的変更やドメイン固有語の取りこぼしは残る。
  - 同義語マップが小さいため、未知語彙では deny ヒステリシス経由の解除にフォールバックする。

#### 9.2 状態遷移 (簡易モデル)

`locked(rule)` のとき、変更 `Δ` に対して次を評価する:

1. `keyword_match` (`unlock_on_change_keywords`)  
2. `symbol_match` (`unlock_on_change_symbols` + AST identifier 抽出)  
3. `evidence_match` (`normalize(tokens(Δ)) ∩ lock_evidence_terms(rule) != ∅`)

いずれかが真なら即時アンロック。すべて偽なら lock 維持し、deny 発生時はヒステリシスで段階解除する。

この順序により、既存設定との後方互換を保ちつつ、`lock_evidence_terms` を追加シグナルとして扱う。
...
- 実装済み:
  ...
  - ルール frontmatter の `severity` を watch 優先度に反映する制御。
  - 直近 queue state の `severity` を watch 優先度に反映する制御。
  - `violations/results` の直近ステータス比率 (`pending`/`resolved`) を watch 優先度に反映する制御。
  - `watch-priority-stats.json` の履歴統計を TTL 付きで参照する優先度制御 (`--watch-history-ttl-seconds`)。
  - 履歴窓 (`trend_seen_count`) による優先度昇格 (短時間の反復変更を high/medium へ引き上げ)。
  - キーワード検知ベースの 3 段階優先度維持 (high/medium/normal)。
  - 遅延キュー復帰時も高優先シグネチャを先に復帰する。
  - 実検出結果連動バックプレッシャー: `pending` 比率に応じて watch の実効キュー上限を動的に縮小する。
  - `--watch-backpressure-high-ratio` / `--watch-backpressure-medium-ratio` で縮小率を調整できる。
- 未実装:
  - ルール本文の意味評価まで含めた適応的優先度制御 (現状はステータス比率ヒューリスティクス)。
...
- 実装済み:
  ...
  - shadow 比較からの設定採否提案 JSON 出力 (`shadow_recommendation_*`)
  - `--approve-shadow-recommendation` による承認付き反映 (`rule-config.json` への `__harness__/selected_profile` 更新と `decision_log` 追記)
  - 承認時のルール単位マージ反映 (`rule_recommendations` -> `rule-config.rules[<rule_key>]`)
  - `rule_metrics` + `evidence_metrics` ベースの `rule_recommendations` 自動生成 (F1/disruption/support の guardrail 付き)
  - `evidence_metrics` 不足時の rule 粒度フォールバックと、`rule_metrics` 未提供時のブロードキャスト フォールバック
  - `shadow_recommendation` の raw 根拠比較出力 (`evidence_comparison`)
  - `evidence_comparison.samples` による集約元サンプル追跡 (`fixture/file/expected/predicted/message`)
  - decision 単位の設定変更監査ログ (`append_rule_config_decision`)
  - cross-file 再チェック拡大 (`cross_file: true` + `dependency_scope: "python_imports" | "python_imports_direct"`)
  - watch モード最小実装 (`--watch`)
  - watch バックプレッシャー最小実装 (キュー上限 + 遅延再挿入)
  - watch の severity 連動優先度制御 (rule frontmatter の `severity`)
  - watch の queue severity 連動優先度制御 (recent queue state)
  - watch の履歴統計優先度制御 (`watch-priority-stats.json`, TTL 付き)
  - watch の実検出結果優先度制御 (`violations/results` の pending 比率)
  - watch の実検出結果連動バックプレッシャー (pending 比率に応じた実効 queue 上限縮小)
  - watch バックプレッシャー比率の運用調整 (`--watch-backpressure-high-ratio`, `--watch-backpressure-medium-ratio`)
  - watch の履歴窓トレンド優先度昇格 (`trend_seen_count`)
  - watch の優先度保持ヒューリスティクス (3 段階キーワード優先度 + 高優先復帰順)
  - lock_on_satisfy のアンロック・ヒステリシス (`--lock-unlock-hysteresis`)
  - lock_on_satisfy の変更起点アンロック (`unlock_on_change_keywords`)
  - lock_on_satisfy の Python AST 識別子抽出アンロック (`unlock_on_change_symbols`)
  - lock_on_satisfy の evidence terms 交差アンロック (`lock_evidence_terms`)
  - dynamic ハーネスの反復上限付き fixpoint ループ (`--max-fixpoint-iterations`)
  - 振動検出 (`state signature`) と `manual_review_required` 出力
- 方針 (未実装):
  - shadow 推奨結果 (`rule_recommendations`) の意味解析ベース高精度化 (現状は evidence category + rule-level confusion/F1/disruption による統計選別)
  - watch モードの意味解析ベース適応バックプレッシャー (現状は pending 比率ベース)
  - lock_on_satisfy の厳密意味解析ベース即時アンロック (AST/span predicate invalidation)

### 14. 誤解しやすいポイント (実装前に必読)

1. `batching` / `context_level` / `cache` について
- これらは `check_style.py` の実行経路を直接切り替える。
- ただし最適化効果は入力特性に依存するため、`tests/test_harness.py` の比較で効果確認する。

2. `lock_on_satisfy` について
- 現行 dynamic ハーネスでは、`--lock-unlock-hysteresis` の連続 deny 閾値でアンロックする。
- 閾値未満の deny はロック維持として一時的に `allow` 扱いになる。
...
- 判定: 不採用
- 理由: これらは実行経路を切り替えるが、入力特性によって効果が変わる。実装変更または運用設定変更に対するハーネス結果が伴わない「改善済み」主張は無効。
