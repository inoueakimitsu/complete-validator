...
tmux send-keys -t test-cv "env -u CLAUDECODE claude --dangerously-skip-permissions"
sleep 0.5
tmux send-keys -t test-cv Enter
sleep 8

# 3) /plugin で local marketplace update/install
#    Marketplaces -> complete-validator -> u(update)
#    Enter -> Browse plugins -> Install
#    Installed -> complete-validator -> i(update)
...
tmux send-keys -t test-cv Enter
sleep 15
tmux capture-pane -t test-cv -p -S -120
```
