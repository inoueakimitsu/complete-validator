...
- 未実装:
  - 「ロック根拠部分に変更が重なった場合のみ即時アンロック」のような差分根拠ベース判定。
...
- 実装済み:
  ...
  - キーワード検知ベースの優先度維持 (security/auth/token/password 等を含む diff を高優先で保持)。
- 未実装:
  - ルール severity と連動した厳密な優先度制御。
...
- 実装済み:
  ...
  - watch の優先度保持ヒューリスティクス (security/auth/token/password 等のキーワード検知)
...
- 方針 (未実装):
  ...
  - lock_on_satisfy の差分根拠ベース即時アンロック
