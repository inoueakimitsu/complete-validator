...
- `tests/test_harness.py` は最小自動生成として、候補 config (`default_model`, `context_level`, `batching`, `cache`) から `rules/**/*.md` 全キーへの `rule_recommendations` を生成する。
- これは「承認付きの最小反映」であり、ルール単位の自動チューニング反映は行わない。
...
- 現状:
  ...
  - `rule_recommendations` が未指定の場合は、候補 config をルールキー集合へブロードキャストして最小提案を自動生成する。
- 方針:
...
- 実装済み:
  ...
  - 候補 config からの `rule_recommendations` 最小自動生成 (`rules/**/*.md` を対象)
...
- 方針 (未実装):
  - shadow 推奨結果 (`rule_recommendations`) の高精度自動生成 (現状は候補 config のブロードキャスト近似)
