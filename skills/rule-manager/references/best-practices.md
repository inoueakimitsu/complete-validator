...
3. **上記で解消できない場合のみ** suppression を追加します。
...
- ルールを修正して偽陽性が解消されたら、該当する suppression を削除します。
- 不要になった suppression が残っていても害はありませんが、メンテナンス性が低下します。

## ルール ファイルの命名規則

- スネーク ケースを使用します (`python_style.md`、`japanese_comment_style.md`)
- 内容を端的に表す名前にします。
- 基本パターンは `{言語}_{関心事}.md` です。
- 言語横断のルールは関心事のみにします (`naming_convention.md` など)。
- ファイルはアルファベット順に読み込まれます。実行順序には影響しません。
