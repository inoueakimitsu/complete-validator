...
- **推奨**: 5-15 個
- **上限目安**: 20 個を超えたら分割を検討する
- **最小**: 1 個でも問題ない (特定の関心事に特化したファイル)
...
**悪い例**: 「命名規則とインデント」を 1 つのルールにまとめる
**良い例**: 「変数の命名規則」と「インデント スタイル」を別々のルールにする
...
- **最低**: 1 ペア (必須)
- **理想**: 2-3 ペア (異なるパターンをカバー)
- **上限**: 4-5 ペア (多すぎるとプロンプトが肥大化する)
...
1. **ルールの修正を検討する** — ルール説明に例外条件を追加することで解消できないか確認する
2. **Bad/Good 例の追加を検討する** — 例を追加することで AI の判定精度を改善できないか確認する
3. **上記で解消できない場合のみ** suppression を追加する

### suppression の粒度

- **具体的に記述する** — どのルールファイルのどのルールに対する抑制かを明記する
- **理由を記述する** — なぜ偽陽性と判断したかを記述する
- **対象を限定する** — 可能な限り対象ファイルやコードのパターンを限定する
